...
Pstax internals - notes for developers

=head1 INTRODUCTION

If you're intending to hack on Pstax, these notes may help ...

The application uses an MVC(ish) architecture in which the Model is implemented
in L<Pstax::Machine>, the View is L<Pstax::GtkView> and the two are tied
together by the L<Pstax> class itself.

A Pstax 'machine' is comprised of 'gears'.

The L<Pstax::Machine> class implements the logic for connecting the gears as
well as a simple event-driven scheduler.

Gears inherit from L<Pstax::Gear.>  A gear passes data downstream to the next
...
A simple gear to look at for a start is L<Pstax::Gear::UpperCase> which simply
takes every line of input and converts it to upper case before passing it to
the next gear.

A more elaborate example is L<Pstax::Gear::Grep> which passes through any lines
...
by the L<Pstax::Gear> base class.
...
L<Pstax::GtkGearView> and implements your gear's user interface.  Your gear
class would override the C<view_subclass()> method and have it return the name
of your view class.  (See L<Pstax::Gear::TextWindow> and
L<Pstax::GtkGearView::TextWindow> for example).
...
The PAD framework is implemented in the L<Pstax::GtkGearView> and
L<Pstax::GtkAutoDialog> classes.

=head1 SCHEDULER

The L<Pstax::Machine> class implements a simple scheduler based around the Gtk
event loop.
